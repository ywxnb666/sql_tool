    ...
    def _get_base_response(self, url, method='GET', param_key=None):
        ...
        try:
            if method.upper() == 'GET':
                ...
                response = self.session.get(base_url, timeout=5, allow_redirects=False)
            else:  # POST
                # 空POST数据或只包含空参数字段
                base_data = {}
                if param_key:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if record_io:
                    ...
                    # 提取关键差异
                    changes = extract_page_changes(base_content, full_content)
                    # 格式化差异为字符串
                    added_str = "\n".join(changes['added']) if changes['added'] else "无"
                    # removed_str = "\n".join(changes['removed']) if changes['removed'] else "无"
                    # key_differences = f"添加的内容:\n{added_str}\n删除的内容:\n{removed_str}"
                    key_differences = f"{added_str}"
                    ...
                    response_info = {
                        ...
                        # 添加差异标记，便于GUI显示
                        'has_differences': len(changes['added']) > 0 or len(changes['removed']) > 0
                    }
...
def extract_page_changes(text_a, text_b, scanner=None, output_file=None):
    ...
    返回:
        dict: 包含差异详细信息的字典
    """
    content_a = set(text_a.splitlines())
    content_b = set(text_b.splitlines())

    changes = {
        'added': list(content_b - content_a),
        'removed': list(content_a - content_b)
    }
